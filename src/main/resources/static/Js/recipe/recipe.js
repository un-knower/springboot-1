...
function initDatagrid() {
    ...
    var columns = [
        [
            ...
            {field: 'tId',title: 'tId',halign: 'center',align: 'center',hidden:'true'},
            {field: 'schoolId',title: '所在学校',width: 150,halign: 'center',align: 'center',
                formatter: schoolFormat},
            {field: 'weekly',title: '时间',width: 100,halign: 'center',align: 'center',
                formatter: function(value, row, index){if(value == "1"){return "周一";}else if(value == "2"){return "周二";}
                else if(value == "3"){return "周三";}else if(value == "4"){return "周四";}else if(value == "5"){return "周五";}
            } },
            {field: 'mealType',title: '类型',width: 100,halign: 'center',align: 'center',
                formatter: function(value, row, index){if(value == "1"){return "正餐";}else if(value == "0"){return "甜品";}
                } },
            {field: 'foodName',title: '名称',width: 150,halign: 'center',align: 'center'},
            {field: 'photo',title: '照片',width: 200,halign: 'center',align: 'center',formatter:formatterHeadUrl},
            {field: 'foodDesc',title: '描述',width: 300,halign: 'center',align: 'center'},
            {field: 'createDate',title: '创建时间',width: 130,halign: 'center',align: 'center',
            ...
            {field: ' ',title: '操作',width: 150,halign: 'center',align: 'center',formatter: function(value, row, index) {
                return '<a onclick="editInfoDo(\''+row.id+'\',\''+row.tId+'\')" class="bootstrap_up_btn" >修改</a><a  onclick="deleteInfoStr(\''+row.tId+'\')" class="bootstrap_del_btn" >删除</a>';
            }
            }
        ]
    ];
    var url = '/recipe/getRecipeList';
    $('#data_dg').datagrid({
        url: url,
        height: 'auto',
        method: 'get',
...
/*新增教师管理*/
function addInfo() {
    if($("#data_dialog").length == 0) { //因为关闭的时候，是强制销毁的，没有这个3行代码，再次点击新增或修改时，时不能创建dialog
        $(document.body).append("<div id='data_dialog'></div>");
    }
    var href = '/recipe/openAddView';
...
function editInfo() {
    ...
    if(selectedRows.length ==0){
        ...
        editInfoDo(id,selectedRows[0].tId);
    }
}


/*编辑*/
function editInfoDo(id,tId) {
    if(!id){
        $.messager.alert("系统提示","您没有可编辑的数据，请选择一条记录！");
    }else{
        ...
        var href = '/recipe/openEditView?id='+id+'&tId='+tId;
        var content = '<iframe id="dataIframe" src="' + href + '" width="100%" height="99%" frameborder="0" scrolling="auto"></iframe>';
        $('#data_dialog').dialog({
            title: '编辑',
            width: 1000,
