...
$(function(){
    $("#mealType").find("option[value="+$("#mealTypeHidden").val()+"]").attr("selected","selected");
    $("#weekly").find("option[value="+$("#weeklyHidden").val()+"]").attr("selected","selected");
    //初始化富文本
    UE.getEditor('editor', {
        toolbars: [
            [
...
function submitFormSave(){
    if($.trim($("#foodName").val())==""){
        $.messager.alert("错误消息","食物名称不能为空！");
        return false;
    }
    clearInterval(tempInveral);
    if(List==null||List.length==0){
        strArray.push($("#img").attr("src"));
        $('#photo').val(JSON.stringify(strArray));
    }else{
        $('#photo').val(JSON.stringify(List));//拿到后台传回来的id，给图片的id赋值序列化表单用
    }
    if(UE.getEditor('editor').hasContents() == true){
        //只取ueditor的纯文本内容存进数据库，
        $('#foodDesc').val($('#foodDesc').html(UE.getEditor('editor').getContentTxt()).text());
        $.ajax({
            url: '/recipe/recipeEdit',
