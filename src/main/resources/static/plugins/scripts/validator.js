...
$.fn.Validform = function () {
    ...
    $(this).find("[isvalid=yes]").each(function () {
        ...
        if (checkexpession != undefined) {
            ...
            switch (checkexpession) {
                ...
                case "NumOrNull":
                    {
                        if (!isIntegerOrNull(value)) {
                            Validatemsg = errormsg + "必须为数字并且不能为空！\n";
                            Validateflag = false;
                            ValidationMessage($(this), Validatemsg); return false;
                        }
                        break;
                ...
                case "LenStrOrNull":
                    {
                        if (!isLenStrOrNull(value, $(this).attr("length"))) {
							Validatemsg = errormsg + "不超过" + $(this).attr("length") + "位字符，并且不能为空！\n";
                            Validateflag = false;
                            ValidationMessage($(this), Validatemsg); return false;
                        }
                        break;
                ...
                case "DateOrNull":
                    {
                        if (!isDateOrNull(value, $(this).attr("eqvalue"))) {
                            Validatemsg = errormsg + "为必填项并且必须为日期格式！\n";
                            Validateflag = false;
                            ValidationMessage($(this), Validatemsg); return false;
                        }
                        break;
                ...
                case "TimeOrNullHours":
                    {
                        if (!isTimeOrNullHours(value, $(this).attr("eqvalue"))) {
                            Validatemsg = errormsg + "必填项并且必须为时间格式(例 09:09)！\n";
                            Validateflag = false;
                            ValidationMessage($(this), Validatemsg); return false;
                        }
                        break;
  ...
  //判断日期类型是否为hh:mm格式的类型 time或者null,空
    function isTimeOrNullHours(obj) {
        var controlObj = $.trim(obj);
        if (controlObj.length == 0 || controlObj == null || controlObj == undefined) {
            return true;
        }
...
function ValidationMessage(obj, validatemsg) {
    try {
    	console.log(obj);
    	$.messager.alert("系统提示", validatemsg);
        //removeMessage(obj);
    	$(".messager-body").window('open');	
      	setTimeout(function(){ 
